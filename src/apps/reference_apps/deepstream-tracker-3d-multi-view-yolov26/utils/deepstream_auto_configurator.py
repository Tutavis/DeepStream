...
class DeepStreamAutoConfigurator:
    ...
    def copy_static_configs(self, detector_config: str = None, num_videos: int = None):
        ...
        if src_path.exists():
            ...
            if num_videos is not None:
                self._scale_batch_size(dst_path, num_videos)
        else:
            print(f"Warning: Template file not found: {src_path}")

    def _scale_batch_size(self, config_path: Path, num_videos: int):
        """Rewrite batch-size= and any _bN_ engine-filename suffix in a copied
        detector config to match the actual number of video sources.

        Some detector templates (e.g. config_pgie_yolo26n.txt/yolo26m.txt) hard-code
        both a fixed batch-size and a model-engine-file with a baked-in _b4_ (or
        other count) suffix, sized for the dataset they were originally written
        against. generate_deepstream_config() already patches config_deepstream.txt's
        own [primary-gie] batch-size/model-engine-file to match num_videos, but
        config_pgie.txt (the low-level nvinfer property file referenced via
        config-file=) is otherwise copied verbatim -- if a stale-batch-size engine
        for a *different* video count happens to already exist on disk, nvinfer can
        deserialize it successfully while streammux delivers a differently-sized
        batch, causing a real (and not necessarily loud) batch-size mismatch at
        inference time rather than a clean rebuild. Mirrors the same substitution
        generate_deepstream_config() already applies.
        """
        content = config_path.read_text()
        content = re.sub(r'^(batch-size=)\d+', rf'\g<1>{num_videos}', content, flags=re.MULTILINE)
        content = re.sub(r'_b\d+_gpu', f'_b{num_videos}_gpu', content)
        config_path.write_text(content)
        print(f"Scaled batch-size/_bN_ engine suffix in {config_path} to {num_videos}")
...
def main():
    ...
    try:
        ...
        configurator.copy_static_configs(args.detector_config, num_videos=len(video_files))
